...
`;
		...
		function SkillsSection({ t }) {
			...
			const badge = (row) => {
				...
				if (row.symlink === true) marks.push(jsx("span", { className: `${styles.badge} ${styles.badgeWarn}`, children: t("badgeSymlink") }, "symlink"));
				marks.push(
					jsx(
						"span",
						{
												...
												open
													? jsxs("div", {
															...
															children: [
																...
																// Diagnostics the row carries (invalid frontmatter, symbolic link,
																// a missing file): shown rather than hidden, so a row that will never
																// reach the catalog explains itself.
																Array.isArray(row.notes) && row.notes.length > 0
																	? jsxs("dl", {
																			className: styles.field,
																			children: [
																				jsx("dt", { children: t("notes") }),
																				jsx("dd", { children: row.notes.join(" ") }),
																			],
																		})
																	: null,
															],
