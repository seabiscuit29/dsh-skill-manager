...
import { hiddenZone, manifestPath, skillsRoot } from "./paths.js";
...
export function buildRows({
	...
	// 4. catalog-only rows: skills the deployment publishes from a root this
	// manager does not own (project root, ~/.agents/skills, bundled). They are
	// shown so the page keeps parity with what the model actually sees, and stay
	// read-only because moving files outside the managed root is not our call.
