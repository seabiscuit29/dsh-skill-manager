...
export function listContainerEntries(dir) {
	...
	for (const entry of readdirSync(dir, { withFileTypes: true })) {
		...
		if (entry.isSymbolicLink()) {
			// The provider reads entries with lstat semantics (`dsh-fs-local` reports
			// "symlink") and only accepts real directories or `.md` files, so a symlinked
			// skill is never published. Surfacing it beats dropping it silently — symlinking
			// a dev checkout into the skill root is an idiomatic move on macOS.
			entries.push({ diskName: entry.name.replace(/\.md$/i, ""), path, form: "symlink", symlink: true });
			continue;
		}
...
export function buildRows({
	...
	const ensure = (name) => {
		if (!rows.has(name)) {
			rows.set(name, {
				...
				// Set when the on-disk entry is a symbolic link: the provider ignores it,
				// so the view must say so rather than pretend it is a working skill.
				symlink: false,
				source: null,
				ref: null,
				path: undefined,
	...
	for (const [state, dir] of [
		...
		for (const item of items) {
			...
			if (item.symlink === true && row.path !== undefined && row.form !== "symlink") continue;
			...
			if (item.symlink === true) {
				row.symlink = true;
				row.notes.push(
					"symbolic link: the provider only publishes real directories or .md files, so this entry never appears in the catalog — replace it with a real copy (or migrate it from the other root)",
				);
			}
			row.path = item.path;
			row.state = state;
...
export function buildDoctor({ root = skillsRoot(), zone = hiddenZone(), catalog } = {}) {
	...
	const symlinked = (view?.rows ?? []).filter((row) => row.symlink === true).map((row) => row.name);
