...
export function stagingRoot() {
	...
	// os.tmpdir() honours TMPDIR on macOS/Linux and TEMP on Windows, and always names a
	// writable volume; a staging → skill-root move that crosses volumes degrades to copy.
	return env && env.trim() !== "" ? env : join(tmpdir(), "dsh-skill-manager");
}
