...
export function mountSkillRoutes({ webServer, catalog }) {
	...
	/** Catalog summaries, or undefined when the registry cannot be read. */
	const readCatalog = async () => {
		try {
			return await catalogOf?.();
		} catch {
			return undefined;
		}
	};
	...
	// Everything the page needs to render: the merged ledger/disk view plus the
	// read-only rows coming from other roots via the catalog.
	route("/list", ["GET"], async (request, response) => {
		const view = buildRows({ catalog: await readCatalog() });
	...
	route("/doctor", ["GET"], async (request, response) => {
		sendJson(response, 200, { ok: true, ...buildDoctor({ catalog: await readCatalog() }) });
	});
